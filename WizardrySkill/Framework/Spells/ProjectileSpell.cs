    ...
    public class ProjectileSpell : Spell
    {
        ...
        *********/
        ...
        public override IActiveEffect OnCast(Farmer player, int level, int targetX, int targetY)
        {
            ...
            Vector2 velocityTowardPoint = Utility.getVelocityTowardPoint(GetShootOrigin(player), AdjustForHeight(new Vector2(targetX, targetY)), (float)(15 + Game1.random.Next(4, 6)) * (1f + player.buffs.WeaponSpeedMultiplier));
        ...
        public float GetRotation(Farmer player, int targetX, int targetY)
        {
            ...
            Vector2 shootOrigin = GetShootOrigin(player);
            float rotation = (float)Math.Atan2((float)mouseY - shootOrigin.Y, (float)mouseX - shootOrigin.X) + ((float)-Math.PI / 2);
