...
namespace WizardrySkill.Core.Framework
{
    ...
    public class SpellBookData
    {
        ...
        *********/
        ...
        private const string UpdatedKey = Prefix + "/Updated";

        /// <summary>The data key for the player's free spell points.</summary>
        private const string FreePointsKey = Prefix + "/FreePoints";

        /// <summary>The data key for the player's known spells.</summary>
        private const string KnownSpellsKey = Prefix + "/KnownSpells";

        /// <summary>The data key for the player's prepared spells.</summary>
        private const string PreparedSpellsKey = Prefix + "/PreparedSpells";

        /// <summary>The data key for the player's selected bar of prepared spells.</summary>
        private const string SelectedBarKey = Prefix + "/SelectedBar";
        ...
        *********/
        ...
        {
            int updatedTick = this.Data.GetInt(UpdatedKey);

            if (this.UpdatedTick != updatedTick)
            {
                this.FreePoints = this.Data.GetInt(FreePointsKey, min: 0);
                this.KnownSpells = this.Data.GetCustom(KnownSpellsKey, parse: this.ParseKnownSpells, suppressError: false) ?? new Dictionary<string, PreparedSpell>();
                this.Prepared = this.Data.GetCustom(PreparedSpellsKey, parse: this.ParsePreparedSpells, suppressError: false) ?? new List<PreparedSpellBar>();
                this.SelectedPrepared = this.Data.GetInt(SelectedBarKey, min: 0);
        ...
        public void Save()
        {
            this.Data.SetInt(FreePointsKey, this.FreePoints, min: 0);
            this.Data.SetCustom(KnownSpellsKey, this.KnownSpells.Values, serialize: this.SerializeSpells);
            this.Data.SetCustom(PreparedSpellsKey, this.Prepared, serialize: this.SerializeSpellBars);
            this.Data.SetInt(SelectedBarKey, this.SelectedPrepared, min: 0);
            this.Data.SetInt(UpdatedKey, Game1.ticks);
        }
